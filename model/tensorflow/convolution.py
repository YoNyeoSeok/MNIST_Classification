...
class CNN():
    ...
    def construct_model(self, initializer='truncated_normal', 
            norm='batch_norm', activation=tf.nn.sigmoid):
        ...
        self.hiddens = [normalize(self.inp @ self.ws[0] + self.bs[0], norm, activation)]

        for i in range(1, len(self.hidden_layers)):
            self.ws += [tf.get_variable('w'+str(i), 
                shape = [self.hidden_layers[i-1], self.hidden_layers[i]],
                dtype = tf.float32, initializer = self.initializer)]
            self.bs += [tf.get_variable('b'+str(i), 
                shape = [self.hidden_layers[i]],
                dtype = tf.float32, initializer = tf.initializers.zeros)]
            self.hiddens.append(normalize(self.hiddens[-1] @ self.ws[i] + self.bs[i], norm, activation))

        self.ws += [tf.get_variable('w'+str(len(self.hidden_layers)), 
            shape = [self.hidden_layers[-1], self.output_dim],
            dtype = tf.float32, initializer = self.initializer)]
...
def normalize(inp, norm, activation):
    if norm == 'batch_norm':
